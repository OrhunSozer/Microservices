...
namespace FreeCourse.Services.Catalog.Services
{
    public class CourseService : ICourseService
    {
        ...
        public async Task<Response<NoContent>> UpdateAsync(CourseUpdateDto courseUpdateDto)
        {
            ...
            var result = await _courseCollection.FindOneAndReplaceAsync<Course>(x => x.Id == updateCourse.Id, updateCourse);
            if (result == null)
                return Response<NoContent>.Fail("Course not found", 404);

            //Publish event using masstransit for other microservices
            await _publishEndpoint.Publish<CourseNameChangedEvent>(new CourseNameChangedEvent()
            {
                CourseId = updateCourse.Id,
                CourseName = courseUpdateDto.Name,
